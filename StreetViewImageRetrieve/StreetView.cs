...
namespace StreetViewImageRetrieve
{
    ...
    {
        public static void GetImages(float latitude, float longitude)
        {
            ...
            using (var client = new HttpClient(new HttpClientHandler()))
            {
                ...
                var x = Regex.Matches(s, "\\[[0-9]+,\"(.+?)\"\\].+?\\[\\[null,null,(-?[0-9]+.[0-9]+),(-?[0-9]+.[0-9]+)");
                var d = Regex.Matches(s, "([0-9]?[0-9]?[0-9])?,?\\[(20[0-9][0-9]),([0-9]+)\\]");
                //"https://cbk0.google.com/cbk?output=tile&panoid={}&zoom={}&x={}&y={}"
            ...
            foreach (var item in panoIds)
            {
                for (int i = 0; i < 2; i++)
                {
                    for (int j = 0; j < 2; j++)
                    {
                        var v = item.Replace("\"", string.Empty);
                        var u = string.Format("https://cbk0.google.com/cbk?output=tile&panoid={0}&zoom={1}&x={2}&y={3}", v, 2, i, j);
                        ...
                        using (var client = new HttpClient(new HttpClientHandler()))
                        {
                            ...
                            if (response.StatusCode == System.Net.HttpStatusCode.OK)
                            {
                                ...
                                File.WriteAllBytes("C:\\Images\\" + v + "_" + i + "_" + j + ".jpg", s);
                            }
                        }
                    }
                }
